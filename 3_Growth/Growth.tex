...
\begin{figure}
    \centering
    \subcaptionbox{
        Cross-section (cut marked in red in the insert showing the topd down view of the structure) of a multi-seeded coalesced III-V structure grown on Si.
        \label{subfig:SM_overview}
    }{
        \tikzsetnextfilename{SM_overview}
        \begin{tikzpicture}
            \node[inner sep=0pt] (image) at (0,0) {\includegraphics[width = 0.98\textwidth]{3_Growth/2020_Merge/JEOL_BF_150k_SOU76posB_Tunnel5Overview.png}};

            \begin{scope}[shift={(5.5,-2)}, >=stealth]
                \filldraw [white] (-0.2, -1.3) rectangle (2, 1.066);
                \draw (0,0) -- ++ (1, 0) -- ++ (-45:0.045255) -- ++ (0.768, 0) -- ++ (0, 0.666) -- ++ (-0.768, 0) -- ++ (-135:0.045255) -- ++ (-1, 0) -- ++ (0, -0.07) -- ++ (1, 0) -- ++ (-45:0.045255) -- ++ (-135:0.045255) -- ++ (-1, 0) -- ++ (0, -0.07) -- ++ (1, 0) -- ++ (-45:0.045255) -- ++ (-135:0.045255) -- ++ (-1, 0) -- ++ (0, -0.07) -- ++ (1, 0) -- ++ (-45:0.045255) -- ++ (-135:0.045255) -- ++ (-1, 0) -- ++ (0, -0.07) -- ++ (1, 0) -- ++ (-45:0.045255) -- ++ (-135:0.045255) -- ++ (-1, 0) -- cycle;
                \draw [red] (1.2, 0.866) -- ++ (0, -1.1);
                \draw [|<->|] (0.25, -1) -- ++ (1.2, 0) node [midway, anchor=south] {\qty{2}{\micro\metre}};
            \end{scope}
            
            \node [white] at (0, 0) {III-V};
            \node at (0, 1) {Template \acs{sio2}};
            \node at (0, -1) {Buried \acs{sio2}};
            \node [white] at (0, 2.15) {Protective \acs{pt}};
        \end{tikzpicture}
    }
    \subcaptionbox{
        Detail of twin planes and dislocations.
...
\begin{figure}
    ...
    \label{subfig:enterprise_etchback}
        ...
        \begin{tikzpicture}
            ...
            \draw (3.2cm, 22mm) arc [start angle = 180, end angle = 305.3, radius = 3mm] node[midway, anchor = north east]{\qty{125.3}{\degree}};
...
While the comb-like design is just meant to grow a group of nanowires close together to facilitate \acf{sem_m} analysis, the T-shaped design was meant to test a theory reliant on the stabilisation of a single \hkl{1 1 0} growth facet. If such a stabilisation was achieved in a layer by layer growth regimen it would mean that, past the initial nucleation region, only \acs{rtp}s containing the growth direction would be able to propagate forward. At the T-junction, if only \hkl{1 1 0} side facets were created, the resulting material in the two arms of the T would be defect-free. This early experimental idea didn't materialise as (as seen later in Figure~\ref{subfig:FIB_cut_2}) the template length were too mismatched in this first experiment to provide the necessary etch back time to result in a single seed, and was not attempted again as experimental findings moved the focus towards the stabilisation of \acs{rtp}-prone \hkl{1 1 1} facets as growth fronts.
...
\begin{figure}
    \centering
    \subcaptionbox{
    Precursor sequence \cite{Brugnolotto2023}.
    ...
    \caption[Precursor sequence and \acs{sem_m} image of sample 1.]{Growth of the first sample. \subref{subfig:recipe1} diagram representing the precursor sequence for a single-heterointerface III-V wire \cite{Brugnolotto2023}. Each line represents an active flow of precursor into the reactor. The colour of the horizontal lines represents the target material. \subref{subfig:comb_sample1} shows a \acs{sem_m} image of an \num{11}-nanowire array. The \acs{SI} seed, III-V segment, empty template, and template opening are visible from left to right.}
...
The \acs{sem_m} image in Figure~\ref{subfig:comb_sample1} shows a comb array with \num{11} nanowires after \acs{mocvd} growth. The structure segments are marked: on the right side of the image, the holes through which the precursors entered the templates are visible. The image was captured with the sample fixed on a stage that was tilted \qty{52}{\degree} with respect to the \hkl<0 0 1> direction in the device layer of the \acs{SOI} device layer before \acs{fib} cutting. The shape of the end facet of each nanowire is visible due to the tilt, with most wires terminating with a \hkl{1 1 1}\(_B\) facet. Only one of the wires, the second one from the top, appears to have a different end-facet. The third wire from the bottom presents an interesting (but not unique, as shown in \cite{Scherrer2022}) situation where the growing crystal leaves the seed partially uncovered. Two \hkl{1 1 0} facets form an arrowhead-like shape, indicating the presence of faster growth in the \hkl<1 1 1> direction compared to the \hkl<1 1 0> directions immediately after nucleation.
...
\begin{figure}
    ...
    \caption[\acs{fib} cut strategy and result for samples grown on \hkl(0 0 1) \acs{SOI}.]{\acs{fib} cut of a lamella from a \hkl<0 0 1> \acs{SOI} wafer. \subref{subfig:FIB_cut} shows the \acs{sem_m} image of a thinned electron-trasparent lamella. The various materials are marked, and the structure is discernible. \subref{subfig:FIB_cut_strategy} shows how the cut is made within the context of the crystalline and growth directions. The red-shaded planes show that the cut was selected to expose the sides of the wire in correspondence to a \hkl{-1 1 0} plane, if the template axis was labelled as a \hkl<1 1 0> direction. \subref{subfig:FIB_cut} was adapted from the work of Brugnolotto \textit{et al.} \cite{Brugnolotto2023} under the terms of the CC BY 4.0 licence \cite{CCBY40}.}
    \label{fig:001_FIB}
\end{figure}

A high-resolution analytical method is needed to analyse the type of structures shown in Figure~\ref{subfig:enterprise_design}. Indeed, their small size means that, for example, X-ray analysis with la\-bo\-ra\-to\-ry-sized tools is unfeasible. Instead, microscopy techniques are particularly powerful, with high-end \acf{tem_m} or \acf{stem_m} able to resolve features up to atomic resolution and provide important structural information. However, an electron-transparent lamella is required to analyse material layers and investigate the morphology of internal heterointerfaces. \Acf{fib} tools can create these <\qty{100}{\nano\metre} thin semiconductor cross-sections. In this study, an FEI Helios NanoLab 450S was used.
...
\begin{figure}
    ...
    \subcaptionbox{
        ...
        \begin{tikzpicture}
            ...
            \node at (4.5, -2.4) {Buried \acs{sio2}};
            \node at (-4, 2.4) {Template \acs{sio2}};
            \node [text=white] at (0, -0.7) {\acs{ingaas}};
            \node [text=white] at (3.5, 1.5) {\acs{inp}};
            \node at (-4.5, 0) {\acs{SI} seed};
            \node at (5, -0.5) {Void};
            \draw [red, stealth-] (2,-2.5) -- ++ (-4,0) node [midway, anchor=south] {\textcolor{red}{Growth direction}};
        \end{tikzpicture}
    }
    \subcaptionbox{
    ...
    \subcaptionbox{
        \acs{eds} map: red-\acs{as}, green-\acs{p}/\acs{pt}, blue-\acs{in}.
        \label{subfig:s1_V_EDS}
    }{
        \includegraphics[width=0.45\textwidth]{3_Growth/s1_V_EDS.pdf}
    }
    \caption[Overview, composition analysis, and detail of the seed/\acs{ingaas} interface in sample 1.]{\subref{subfig:sample1_cross-sec} overview of a III-V nanowire grown in a comb array. The wire grew from the \acs{SI} seed on the left in the direction indicated by the red arrow. The red line highlights the shape of the \acs{ingaas} / \acs{inp} heterointerface, and the orange line indicates the shape of the end facet of the wire. The dashed lines indicate the presence of a facet which does not contain the viewing direction in the \acs{ingaas} / \acs{inp} heterointerface. The yellow square shows the position of Figure~\ref{subfig:si-ingaas_s1}. \subref{subfig:s1_III_EDS} and \subref{subfig:s1_V_EDS} show the \acs{eds} composition maps of sample 1.}
    \label{fig:sample1}
\end{figure}
...
\begin{figure}
    ...
    \begin{tikzpicture}
        \node[inner sep=0pt] (image) at (0,0) {\includegraphics[width=0.4\textwidth]{3_Growth/Si_interface_sample1.pdf}};
        \node [white] at (2, -1) {\acs{ingaas}};
        \node at (-2, 2) {\acs{SI} seed};
    \end{tikzpicture}
...
\begin{figure}
    \centering
    \subcaptionbox{
    Precursor sequence for sample 2 \cite{Brugnolotto2023}.
    ...
    \subcaptionbox{
        ...
        \begin{tikzpicture}
            ...
            \draw [red, stealth-] (-6,-2.5) -- ++ (4.5,0) node [midway, anchor=north] {\textcolor{red}{Growth direction 2}};
            \draw [red, stealth-] (6,-2.5) -- ++ (-4.5,0) node [midway, anchor=north] {\textcolor{red}{Growth direction 1}};

            \begin{scope}[scale=0.05, shift={(-70,50)}]
                \filldraw [white] (18,-11) rectangle (40.5, 17);
                \draw (20, 1) -- ++ (10, 0) -- ++ (-26.6:4.445) -- ++ (0, -6) -- ++ (4, 0) -- ++ (0, 20) -- ++ (-4, 0) -- ++ (0, -6) -- ++ (26.6:-4.445) -- ++ (-10, 0) -- cycle;
                \draw[red] (33.5, 11) rectangle (38.5, 13);
                \draw[red] (33.5, -7) rectangle (38.5, -5);
                \draw[cb1_purple, dashed] (36, -9) -- ++ (0, 24);
            \end{scope}
        \end{tikzpicture}
    }
    \caption[Precursor sequence and \acs{sem_m}-\acs{fib} image of a cross-section of a wire from sample 2]{Growth of the second sample. \subref{subfig:recipe1} is the diagram representing the modified precursor sequence introducing the looped segment for thin layer deposition \cite{Brugnolotto2023}. Each line represents an active flow of precursor into the reactor. The colour of the horizontal lines represents the target material. \subref{subfig:FIB_cut_2} \acs{sem_m} image of the lamella during \acs{fib} thinning. The various material regions are labelled, including the \acs{SI} seed, from which \num{2} III-V wires grew towards in opposite directions, highlighted by the red arrows underneath them. The vertical purple dashed line in the insert next to the scale bar shows where the cross-section was cut.}
...
\begin{figure}
    \centering
    \subcaptionbox{
        \acs{bf}-\acs{stem_m} image of the right wire of sample 2 \cite{Brugnolotto2023}. The growth direction is marked in red.
        \label{subfig:s2_r_ov}
    }{
        \tikzsetnextfilename{s2_r_ov}
        \begin{tikzpicture}
            \node[inner sep=0pt] (image) at (0,0) {\includegraphics[width = \textwidth]{3_Growth/S2_right_overview.pdf}};

            \node at (-7, -1) {\acs{SI} seed};
            \node [white] at (-3, 0) {\acs{ingaas}};
            \node [white] at (1, 1) {\acs{inp}};
            \node [white] at (4, 0) {III-V};
            \node at (-6.5, 2.7) {Template \acs{sio2}};
            \node at (0, -3) {Buried \acs{sio2}};
            \draw [red, stealth-] (5, 2.6) -- ++ (-6.5,0) node [midway, anchor=south] {\textcolor{red}{Growth direction}};
        \end{tikzpicture}
    }
    \subcaptionbox{
...
\begin{figure}
    ...
    \subcaptionbox{
        ...
        \tikzsetnextfilename{s2_thick_heterostructure}
        \begin{tikzpicture}
            \node[inner sep=0pt] (image) at (0,0) {\includegraphics[width=0.45\textwidth]{3_Growth/s2_thick_Heterostructure.pdf}};

            \node [fill=white] at (2.5, 3) {\acs{inp}};
            \node [fill=white] at (2.5, -3) {\acs{ingaas}};
            \node [fill=white] at (-2.5, 3) {\acs{ingaas}};
        \end{tikzpicture}
    }
    \subcaptionbox{
        ...
        \tikzsetnextfilename{s2_V_thin_heterostructure}
        \begin{tikzpicture}
            \node[inner sep=0pt] (image) at (0,0) {\includegraphics[width=0.45\textwidth]{3_Growth/s2_thin_Heterostructure.pdf}};

            \node [fill=white] at (0, -1) {III-\acs{p}};
            \node [fill=white] at (2.5, -3) {\acs{ingaas}};
            \node [fill=white] at (-2.5, 3) {\acs{ingaas}};
        \end{tikzpicture}
    }
    \subcaptionbox{
        \acs{eds} map of the \acs{p} concentration. In the bottom right, \acs{pt} signal affects this map.
    ...
    \caption[High-resolution \acs{bf}-\acs{stem_m} images and \acs{eds} maps of \acs{in} and \acs{p} in the area grown to contain thin material layers in sample 2.]{Analysis of the thin-layer region. \subref{subfig:s2_thick_heterostructure} \acs{bf}-\acs{stem_m} image of the lower part of the thick \acs{inp} layer, where it is at its thinnest. \subref{subfig:s2_V_thin_heterostructure} \acs{bf}-\acs{stem_m} image of the thin phosphide interface at the bottom left of \ref{subfig:s2_r_ov}. \subref{subfig:s2_int2_P} \acs{eds} composition map of \acl{p} presence in the sample. \subref{subfig:s2_int2_In} \acs{eds} composition map of \acl{in} presence in the sample. \subref{subfig:s2_thick_heterostructure} and \subref{subfig:s2_V_thin_heterostructure} were adapted from the work of Brugnolotto \textit{et al.} \cite{Brugnolotto2023} under the terms of the CC BY 4.0 licence \cite{CCBY40}.}
...
\begin{sidewaysfigure}
    ...
    \subcaptionbox{
        Precursor sequence for sample 3 \cite{Brugnolotto2023}. Each line represents an active flow of the corresponding precursor into the reactor. The colour of the horizontal lines represents the target material.
    ...
    \caption[Diffusion avenues in the template and changes to the precursor sequence with the introduction of hold steps for the growth of sample 3.]{\subref{subfig:diffusion_mechanisms} Schematic representation of the diffusion avenues available inside the \acs{TASE} template. \subref{subfig:recipe3} Modified \acs{mocvd} recipe based on the analysis of growth results.}
    \label{fig:s3_hold_steps}
\end{sidewaysfigure}

The stabilisation of the \hkl{1 1 1} facet in high V/III ratio \acs{inp} deposition makes this combination of material and facet the preferred one for the growth of thin heterostructures. The potential for thin heterolayers and \acl{qw}s was highlighted by the \qty{6}{\nano\metre} wide phosphide structures shown in the second sample. The thickness of the \acs{inp} layer in Figure~\ref{subfig:s2_thick_heterostructure} can be misleading, as without the context given by the larger overview image in Figure~\ref{subfig:s2_r_ov}, the information is lost that most of the material deposition occurs on the \hkl{1 1 0} facets. 
...
\begin{figure}
    \centering
    \tikzsetnextfilename{s3_ov}
    \begin{tikzpicture}
        \node[inner sep=0pt] (image) at (0,0) {\includegraphics[width=\textwidth]{3_Growth/s3_ov.pdf}};

        \node at (6, -1) {\acs{SI} seed};
        \node at (-6, 1.5) {Void / \acs{pt}};
        \draw [stealth-] (0.2, 4.5) -- ++ (0, 0.5) node [anchor=south] {\acs{ingaas}};
        \draw [stealth-] (2.3, 4.5) -- ++ (0, 0.5) node [anchor=south] {\acs{ingaas}};
        \node [white] at (3.5, 1) {\acs{ingaas}};
        \node [white] at (0.5, 1) {\rotatebox{60}{\acs{inp}}};
        \node [white] at (-1.5, 1) {\rotatebox{60}{\acs{inp}}};
        \node [white] at (-3.5, 1) {\rotatebox{60}{\acs{inp}}};
        \node at (5, -4) {Buried \acs{sio2}};
        \node at (-5, 5) {Template \acs{sio2}};
        \draw [red, -stealth] (2, -3.5) -- ++ (-5,0) node [midway, anchor=south] {Growth direction};
    \end{tikzpicture}
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \node [fill=white] at (2, 3.5) {\acs{ingaas}};
        \node [fill=white] at (2.5, -3) {\acs{inp}};
        \node [fill=white] at (-3, 3.5) {\acs{inp}};
    \end{tikzpicture}
...
\begin{figure}
    ...
    \subcaptionbox{
        \acs{eds} map: Red-\acs{as} Green-\ce{O} Blue-\acs{p}/\acs{pt}
...
The facet stabilisation method is expected to provide a solution or, at the very least, improve on all the issues identified in Figure~\ref{fig:wen}. However, some critical problems remain. The first is related to the possibility of the stabilisation of either of 2 different \hkl{1 1 1} facets. This negates the advantage gained in terms of contact positioning, as the selection of the \hkl{1 1 1} facet remains a stochastic process. This is an unsolvable problem with the \hkl[0 0 1] \acs{SOI} wafer, as this substrate defines the crystalline orientation of the growing wire. 
...
In particular, there are remaining questions on the reproducibility of the study, the yield of the growth stabilisation, the optical properties of the devices and the application of this facet stabilisation method to the integration of different III-V materials outside of the lattice-matched \ce{In0_.55Ga0_.45As} and \acs{inp} material system \cite{Pearsall1980, Sugii1983, Wagner1970} studied in this chapter.