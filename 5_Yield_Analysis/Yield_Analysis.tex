...
Early during the development of the splitting algorithm, it was found that, due to shading in the \acs{sem_m} image, both vectors had their largest gradients in correspondence to the area which sees the introduction of the template side walls. Furthermore, for the initial, smaller dataset used in the early trials, it was also found that taking the largest gradient further down the vector resulted in a tighter cut. Unfortunately, as seen in the plot in Figure~\ref{fig:pooling} this had to be changed in the code revisions carried out after the initial classifier training to include a handful of images that presented this problem and would otherwise be excluded in the input dataset.
