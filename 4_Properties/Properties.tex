...
\begin{figure}
    ...
    \label{subfig:110_etchback}
        ...
        \begin{tikzpicture}
            ...
            \draw (3.7cm, 22mm) -- ++ (0, -3mm) node[anchor = north east]{\qty{90}{\degree}} -- ++ (3mm, 0);
...
\par
Figure~\ref{subfig:110_etchback} shows a schematic cross-section of the \acs{TASE} structure before growth with colour-coded materials. The \acl{SI} seed presents a \hkl{1 1 1} facet forming a \qty{90}{\degree} angle with all the template walls because of the \acs{tmah} etchant used to remove the sacrificial \acl{SI}; unlike the \hkl<0 0 1> wafer samples which formed \qty{90}{\degree} angles with the sidewalls (Figure~\ref{subfig:enterprise_etchback}).
    ...
    \label{subfig:110_design2}
        ...
        \begin{tikzpicture}
            ...
            \begin{scope}[shift={(15cm, 0.5cm)}]
                ...
                \draw [stealth-stealth, cb1_orange] (0, 1.5) -- (1, 1.5) node [cb1_orange, midway, anchor = south] {\qty{1}{\micro\metre}};
                \draw [cb1_orange] (0, 1.3) -- (0, 1.7);
                \draw [cb1_orange] (1, 1.3) -- (1, 1.7);
            \end{scope}
            \begin{scope}[shift={(18cm, 1.5cm)}]
                ...
                \node[mark size=2pt] at (0, 0) {\pgfuseplotmark{square*}};
            \end{scope}
            \begin{scope}[shift={(18cm, 3.7cm)}, rotate=20]
...
Both structures in Figure~\ref{subfig:110_design2} were grown aligned to the in-plane \hkl<1 1 1> direction but also \qty{20}{\degree} misaligned. In practice, this was done by aligning one set of structures to an in-plane \hkl<1 1 1> direction and rotating each subsequent set by \qty{90}{\degree}. Selecting this specific \qty{20}{\degree} misalignment as a basis for analysis therefore has two advantages:
\begin{enumerate}
...
\begin{sidewaysfigure}
    \centering
    \subcaptionbox{
    Precursor sequence for sample 4 \cite{Brugnolotto2023}. Each line represents an active flow of the corresponding precursor into the reactor. The colour of the horizontal lines represents the target material. The dashed lines are time-compressed 10 times. The \acl{qw} growth loop was executed three times.
    ...
    \subcaptionbox{
        \acs{bf}-\acs{stem_m} overview images of two nanowires from sample 4. The wire on the left was cut with a regular cross-section, while the wire on the right was cut with a \qty{30}{\degree} cross-section, as shown by the 3D drawings. The direction guide only refers to the drawings.
        \label{subfig:s4_OV}
    }{
        \tikzsetnextfilename{s4_OV}
        \begin{tikzpicture}
            \node[inner sep=0pt] (image) at (0,0) {\includegraphics[width = 0.48\textwidth]{4_Properties/Fig/s4_OV.pdf}};
            \node[fill=white] at (-5, 2) {(l)};
            \node[fill=white] at (5, 2) {(r)};

            \draw [red, stealth-] (5, -0.9) -- ++ (-4.5, 0) node [midway, anchor=south] {\hkl<-1 1 1> Growth direction};
            \draw [red, stealth-] (-5, -0.4) -- ++ (4.5, 0) node [midway, anchor=south] {\hkl<-1 1 1> Growth direction};

            \begin{scope}[shift={(-4, 3, 0)}, scale=0.4, isometric]
            ...
            \begin{scope}[shift={(0, 2.5, 0)}, scale=0.7, isometric]
                \draw [-stealth] (0, 0, 0) -- ++ (1, 0, 0) node[anchor = north west]{\hkl<-1 1 1>};
                \draw [-stealth] (0, 0, 0) -- ++ (0, 1, 0) node[anchor = south]{\hkl<1 1 0>};
                \draw [-stealth] (0, 0, 0) -- ++ (0, 0.5, 0.866) node[anchor = east]{\hkl<0 1 -1>};
                \draw [-stealth] (0, 0, 0) -- ++ (0, 0.5, -0.866) node[anchor = south west]{\hkl<1 0 1>};
            ...
            \begin{scope}[shift={(3, 3, 0)}, scale=0.4, isometric]
                \draw (0, 0, 0) -- (0, 0, 2) -- (0, 1, 2) -- (0, 1, 0) -- cycle;
                \draw (0, 0, 0) -- (3, 0, 0) -- (3, 1, 0) -- (0, 1, 0);
                \draw (0, 0, 2) -- (3, 0, 2) -- (3, 1, 2) -- (0, 1, 2);
                \draw (3, 0, 0) -- (3, 0, 2);
    ...
    \subcaptionbox{
        High-resolution \acs{bf}-\acs{stem_m} images of the first \acl{qw} for both wires in \subref{subfig:s4_OV} with \acs{fft} inserts. Further \acs{fft} analysis is shown in Figure~\ref{fig:FFT_analysis}.
        \label{subfig:s4_HR}
    }{
        \tikzsetnextfilename{s4_HR}
        \begin{tikzpicture}
            \node[inner sep=0pt] (image) at (0,0) {\includegraphics[width = 0.48\textwidth]{4_Properties/Fig/s4_HR.pdf}};
            \node[fill=white] at (-5.3, 2.3) {(l)};
            \node[fill=white] at (5.3, 2.3) {(r)};
            \node[fill=white] at (-5.3, -1.7) {\acs{inp}};
            \node[fill=white] at (-3.5, -1.7) {\acs{ingaas}};
            \node[fill=white] at (-1, -1.7) {\acs{inp}};
            \node[fill=white] at (1, -1.7) {\acs{inp}};
            \node[fill=white] at (3.5, -1.7) {\acs{ingaas}};
            \node[fill=white] at (5.3, -1.7) {\acs{inp}};
        \end{tikzpicture}
    }
    \caption[Precursor sequence and \acs{bf}-\acs{stem_m} images of sample 4.]{Precursor sequence and \acs{bf}-\acs{stem_m} images of sample 4. The III-V material appears dark due to channelling contrast, with \acs{ingaas} being the darkest of the two III-V materials. \subref{subfig:recipe4} shows the precursor sequence. \subref{subfig:s4_OV} shows overview images of the normal cross-section and \qty{30}{\degree} cross-section. \subref{subfig:s4_HR} shows high-resolution images of the first \acl{qw} for both wires in \subref{subfig:s4_OV} with inserts showing the calculated \acs{fft}s. \subref{subfig:s4_OV}(l) and \subref{subfig:s4_HR}(l) were adapted from the work of Brugnolotto \textit{et al.} \cite{Brugnolotto2023} under the terms of the CC BY 4.0 licence \cite{CCBY40}.}
...
\begin{figure}
    \centering
    \tikzsetnextfilename{FFT_analysis}
    \begin{tikzpicture}
        \node[inner sep=0pt, label=below:{\textcolor{red}{IFFT}}] (0deg_IFFT) at (-4,0) {\includegraphics[scale=0.7]{4_Properties/Fig/0deg_IFFT.pdf}};
        \node[inner sep=0pt, label=above:{\qty{0}{\degree} cut}, label=below:{FFT}] (0deg_FFT) at (0,0) {\includegraphics[width=0.3\textwidth]{4_Properties/Fig/0deg_FFT.pdf}};
        \node[inner sep=0pt, label=above:{\qty{30}{\degree} cut}, label=below:{FFT}] (30deg_FFT) at (5,0) {\includegraphics[width=0.3\textwidth]{4_Properties/Fig/30deg_FFT.pdf}};
        \node[inner sep=0pt, label=below:{\textcolor{red}{IFFT}}] (30deg_IFFT) at (9,0) {\includegraphics[scale=0.9]{4_Properties/Fig/30deg_IFFT.pdf}};

        \draw [red] (-0.25,0.25) rectangle (0.25,0.75);
        \draw [red] (4.4,0.25) rectangle (4.75,0.6);
        \draw [red, -stealth] (-0.25, 0.5) -- (0deg_IFFT);
        \draw [red, -stealth] (4.75, 0.35) -- (30deg_IFFT);
    \end{tikzpicture}
...
\begin{figure}
    ...
    \subcaptionbox{
        ...
        \tikzsetnextfilename{s4_0deg_IIImap_wells}
        \begin{tikzpicture}
            \node[inner sep=0pt] at (0,0) {\includegraphics[width=0.47\textwidth]{4_Properties/Fig/s4_0deg_IIImap_wells.pdf}};

            \draw [red, stealth-] (2.5, -3) -- ++ (-5, 0) node [midway, anchor=south] {Growth direction};
        \end{tikzpicture}
    }
    \subcaptionbox{
        ...
        \tikzsetnextfilename{s4_0deg_Vmap_wells}
        \begin{tikzpicture}
            \node[inner sep=0pt] at (0,0) {\includegraphics[width=0.47\textwidth]{4_Properties/Fig/s4_0deg_Vmap_wells.pdf}};

            \draw [red, stealth-] (2.5, -3) -- ++ (-5, 0) node [midway, anchor=south] {Growth direction};
        \end{tikzpicture}
    }
    \caption[\acs{eds} maps of the \acl{qw} region of sample 4.]{\acs{eds} maps of the \acl{qw} region of the \qty{0}{\degree} cross-section in Figure~\ref{subfig:s4_OV}. The seed is out of frame on the left of the images. This Figure was adapted from the work of Brugnolotto \textit{et al.} \cite{Brugnolotto2023} under the terms of the CC BY 4.0 licence \cite{CCBY40}.}
...
The III group element composition profile of Figure~\ref{subfig:s4_III_0deg_linesc} was calculated using the L\(_\alpha\) lines of \acl{in} and \acl{ga}. The graph is very noisy, with noise fluctuations reaching \qty{20}{\%}, but shows a composition close to \ce{In0_.4Ga0_.6As} in all three \acl{qw}s. Similarly to the profile in Figure~\ref{subfig:s3_III_linesc}, it suggests the presence of a \acl{ga} composition gradient after the well itself. 
...
\begin{sidewaysfigure}
    \centering
    \subcaptionbox{
    Precursor sequence for sample 5 and for merge structures \cite{Brugnolotto2023_2}. Each line represents an active flow of the corresponding precursor into the reactor. The colour of the horizontal lines represents the target material. The dashed lines are time-compressed 10 times. The \acl{qw} growth loop was executed three times.
    ...
    \subcaptionbox{
        ...
        \begin{tikzpicture}
            ...
            \node [rotate=24] at (-4, 1.6) {\acs{SI}};
            \node [rotate=24] at (-3.1, 1.9) {III-V};
            \node [rotate=-43] at (1.9, -2.5) {Template};
            \node [rotate=-43] at (2.4, -2.4) {Opening};
            \node [white, rotate=-43] at (-3, -1) {\acs{SI} backbone};
        \end{tikzpicture}
    }
    \subcaptionbox{
        \acs{bf}-\acs{stem_m} overview image of a nanowire from sample 5. The \acs{SI} seed is on the left and the growth direction is highlited in red.
        \label{subfig:s5_OV}
    }{
        \tikzsetnextfilename{s5_OV}
        \begin{tikzpicture}
            \node[inner sep=0pt] (image) at (0,0) {\includegraphics[width = 0.48\textwidth]{4_Properties/Fig/s5_OV.pdf}};

            \draw [red, stealth-] (4, -2) -- ++ (-5, 0) node [midway, anchor=south] {Growth direction};
            \node at (-5, 0) {\acs{SI}};
            \node at (-5, 1) {\acs{sio2}};
            \node at (5, -1.5) {\acs{sio2}};
            \draw [stealth-] (-2, 0.4) -- ++ (0, 0.5) node [anchor=south] {\acs{inp}};
            \draw [stealth-] (-0.9, 0.4) -- ++ (0, 1) node [anchor=south] {\acs{ingaas}};
            \draw [stealth-] (0, 0.45) -- ++ (0, 0.5) node [anchor=south] {\acs{inp}};
            \draw [stealth-] (0.9, 0.45) -- ++ (0, 1) node [anchor=south] {\acs{ingaas}};
            \draw [stealth-] (1.85, 0.5) -- ++ (0, 0.5) node [anchor=south] {\acs{inp}};
            \draw [stealth-] (2.8, 0.5) -- ++ (0, 1) node [anchor=south] {\acs{ingaas}};
            \draw [stealth-] (3.8, 0.5) -- ++ (0, 0.5) node [anchor=south] {\acs{inp}};
        \end{tikzpicture}
    }
    \caption[\acs{mocvd} recipe and growth results for sample 5.]{\acs{mocvd} recipe and growth results for sample 5. \subref{subfig:recipe5} shows the \acs{mocvd} precursor sequence. \subref{subfig:SEM_array} shows an \acs{sem_m} image of an array of nanowires. The image was recorded with the stage tilted \qty{52}{\degree}. An in-plane \hkl<1 1 0> direction runs parallel to the bottom margin of the image. \subref{subfig:s5_OV} shows a \acs{bf}-\acs{stem_m} overview images of a nanowires. The \acs{SI} seed is on the left of the III-V nanowire.}
    \label{fig:s5_recipe_microscopy}
\end{sidewaysfigure}

After examining the photoluminescence results of sample 4 (Figure~\ref{fig:s4_pl}) the recipe was adjusted to minimise the effect of the \acs{ingaas} nucleation layer. Previous experience in the research group at \acs{ibm} Research Europe - Zurich has shown that this layer, grown with a target composition of \ce{In0_.53Ga0_.47As}, has a positive effect on the nucleation yield of III-Vs on \acl{SI}. However, once nucleation is achieved, it does not serve a further purpose in the experiments described in this chapter. 
...
\begin{figure}
    ...
    \subcaptionbox{
        ...
        \begin{tikzpicture}
            ...
            \draw[|-|, white] (-2.3, 1.8) -- ++ (1.7, 0)  node[midway, anchor=south, white] {\qty{500}{\nano\metre}};
            
            \node [white, rotate=35] at (-1.5, -1.9) {\acs{SI} seeds};
            \node [white, rotate=35] at (0, -0.8) {III-V};
            \node [white, rotate=-55] at (1.2, 1.5) {Opening};
        \end{tikzpicture}
    }
    \subcaptionbox{
        \acs{sem_m} image of a lamella containing merge structures during \acs{fib} plane-view thinning.
        ...
        \begin{tikzpicture}
            ...
            \draw[|-|, white] (1.5, -2) -- ++ (2.2, 0)  node[midway, anchor=south, white] {\qty{500}{\nano\metre}};
            
            \node [white, rotate=5] at (-1, 1.9) {\acs{SI} seeds};
            \node [white, rotate=5] at (-1, 0.8) {III-V};
        \end{tikzpicture}
    }
    \caption[\acs{sem_m} image of merge structures taken before and  during \acs{fib} lamella cut-out and thinning.]{\acs{sem_m} image of merge structures taken \subref{subfig:SEM_merge_bulk} before and \subref{subfig:SEM_merge_lamella} during \acs{fib} lamella cut-out and thinning. The three \acs{SI} seeds seen on the bottom left of \subref{subfig:SEM_merge_bulk} appear at the top of \subref{subfig:SEM_merge_lamella}, which gives a point of view from below the wafer surface now that the lamella has been thinned and the buried \acs{sio2} sputtered away.}
    \label{fig:SEM_merge}
\end{figure}

In addition to the nanowire arrays (Figure~\ref{subfig:SEM_array}), sample 5 contained the multi-seed structures shown in Figure~\ref{fig:SEM_merge}. 
...
The \acs{sem_m} image in Figure~\ref{subfig:SEM_merge_lamella} shows the same structures already seen in Figure \ref{subfig:SEM_merge_bulk}, this time in the final phases of \acs{fib} processing. The lamella was cut using the "plane view" technique (Figure~\ref{fig:110_FIB} left) to access multiple structures simultaneously on a \hkl<1 1 0> viewing axis at the \acs{stem_m}. Three lighter lines are already visible in each structure with \acs{sem_m} at this magnification level likely corresponding to the \acs{ingaas} layers.

\begin{figure}
    \centering
    \tikzsetnextfilename{merge_ov}
    \begin{tikzpicture}
        \node [inner sep=0pt] (image) at (0,0) {\includegraphics[width=\textwidth]{4_Properties/Fig/merge_OV.pdf}};

        \draw[white, decorate, decoration={brace}] (5.3, 0.5) -- ++ (0, 1.5) node[midway, anchor=east]{\rotatebox{90}{\acs{ingaas} wells}};
        \node at (6.5, 3.3) {\acs{SI} seeds};
        \node [white] at (6.5, 2.3) {\acs{inp}};
        \draw [red, stealth-] (1.9, -1) -- ++ (0, 4) node [midway, anchor=east] {\rotatebox{90}{Growth direction}};
    \end{tikzpicture}
    \caption[\acs{bf}-\acs{stem_m} overview image of the lamella containing five merge structures.]{\acs{bf}-\acs{stem_m} overview image of the lamella containing five merge structures. The right most structure has the important material layers labelled. The three structures on the left have seed widths of \qty{50}{\nano\metre} and the two on the right of \qty{70}{\nano\metre}. The \acs{ingaas} wells appear in the same position in all five structures, and all have heterointerfaces consisting of a single \hkl{1 1 1} facet. This Figure was adapted from the work of Brugnolotto \textit{et al.} \cite{Brugnolotto2023_2} under the terms of the CC BY 4.0 licence \cite{CCBY40}.}
    \label{fig:merge_ov}
\end{figure}
...
\begin{figure}
    ...
    \subcaptionbox{
        ...
        \begin{tikzpicture}
            ...
            \node[white] at (3.4, 1.4) {\num{1}};
            \node[white] at (3.4, -0.2) {\num{2}};
            \node[white] at (3.4, -1.9) {\num{3}};

            \draw[decorate, decoration={brace}] (-2.1, 2.4) -- ++ (3, 0) node[midway, anchor=south]{3x \acs{ingaas} wells};
            \node[white] at (2, -0.2) {\acs{inp}};
            \node at (1, -3.2) {Template \acs{sio2}};
        \end{tikzpicture}
    }
    \subcaptionbox{
    ...
    \subcaptionbox{
        ...
        \tikzsetnextfilename{STEM_merge_seed1}
        \begin{tikzpicture}
            \node[inner sep=0pt] (image) at (0, 0) {\includegraphics[width=0.24\textwidth]{4_Properties/Fig/STEM_merge_seed1.pdf}};

            \draw[decorate, decoration={brace}] (-2, -0.6) -- ++ (0, 1.7) node[midway, anchor=east]{\rotatebox{90}{\acs{ingaas}}};
            \draw[decorate, decoration={brace}] (-2, 1.1) -- ++ (0, 0.8) node[midway, anchor=east]{\rotatebox{90}{\acs{SI}}};
            \draw[decorate, decoration={brace, mirror}] (-2, -0.6) -- ++ (0, -1.3) node[midway, anchor=east]{\rotatebox{90}{\acs{inp}}};
        \end{tikzpicture}
    }
    \subcaptionbox{
        Seed 2.
        ...
        \tikzsetnextfilename{STEM_merge_seed2}
        \begin{tikzpicture}
            \node[inner sep=0pt] (image) at (0, 0) {\includegraphics[width=0.24\textwidth]{4_Properties/Fig/STEM_merge_seed2.pdf}};
            
            \draw[decorate, decoration={brace}] (-2, -1.15) -- ++ (0, 1.50) node[midway, anchor=east]{\rotatebox{90}{\acs{ingaas}}};
            \draw[decorate, decoration={brace}] (-2, 0.35) -- ++ (0, 1.5) node[midway, anchor=east]{\rotatebox{90}{\acs{SI}}};
            \draw[decorate, decoration={brace, mirror}] (-2, -1.15) -- ++ (0, -0.75) node[midway, anchor=east]{\rotatebox{90}{\acs{inp}}};
        \end{tikzpicture}
    }
    \subcaptionbox{
        Seed 3.
        ...
        \tikzsetnextfilename{STEM_merge_seed3}
        \begin{tikzpicture}
            \node[inner sep=0pt] (image) at (0, 0) {\includegraphics[width=0.24\textwidth]{4_Properties/Fig/STEM_merge_seed3.pdf}};
            
            \draw[decorate, decoration={brace}] (-2, -1.15) -- ++ (0, 1.35) node[midway, anchor=east]{\rotatebox{90}{\acs{ingaas}}};
            \draw[decorate, decoration={brace}] (-2, 0.2) -- ++ (0, 1.7) node[midway, anchor=east]{\rotatebox{90}{\acs{SI}}};
            \draw[decorate, decoration={brace, mirror}] (-2, -1.15) -- ++ (0, -0.75) node[midway, anchor=east]{\rotatebox{90}{\acs{inp}}};
        \end{tikzpicture}
    }
    \caption[\acs{tem_m} and \acs{stem_m} images of the merge structures of sample 5.]{\subref{subfig:STEM_merge_structure_ov} \acs{bf}-\acs{stem_m} and \subref{subfig:TEM_merge_structure_ov} \acs{df}-\acs{tem_m} overview images of a single merge structure. \acs{rtp}s are more easily identifiable by the thin lines in \subref{subfig:TEM_merge_structure_ov}, but no other defects can be spotted. \subref{subfig:STEM_merge_seed1}, \subref{subfig:STEM_merge_seed2}, and \subref{subfig:STEM_merge_seed3} are \acs{bf}-\acs{stem_m} images of the nucleation areas 1, 2, and 3 as marked in \subref{subfig:STEM_merge_structure_ov}; the \acs{SI} seed, \acs{ingaas} nucleation, and \acs{inp} stabilisation layers are marked. \subref{subfig:TEM_merge_structure_ov} was kindly recorded by Michael Stiefel. \subref{subfig:STEM_merge_structure_ov} was adapted from the work of Brugnolotto \textit{et al.} \cite{Brugnolotto2023_2} under the terms of the CC BY 4.0 licence \cite{CCBY40}.}
...
\begin{figure}
    \centering
    \subcaptionbox{
        \acs{bf}-\acs{tem_m} image of a candidate merging region.
        \label{subfig:merge_region}
    }{
        \tikzsetnextfilename{merge_region}
        \begin{tikzpicture}
            \node[inner sep=0pt] (image) at (0, 0) {\includegraphics[width=0.47\textwidth]{4_Properties/Fig/merge_region.pdf}};

            \node [fill=white] at (2.3, -3) {Template \acs{sio2}};
            \node [fill=white] at (-3, 3) {\acs{inp}};
        \end{tikzpicture}
    }
    \subcaptionbox{
        ...
        \tikzsetnextfilename{merge_qw}
        \begin{tikzpicture}
            \node[inner sep=0pt] (image) at (0, 0) {\includegraphics[width=0.47\textwidth]{4_Properties/Fig/merge_qw.pdf}};

            \node [fill=white] at (2.8, -3) {\acs{inp}};
            \node [fill=white] at (2.8, 0) {\acs{ingaas}};
            \node [fill=white] at (2.8, 3) {\acs{inp}};
            \draw [red, stealth-] (-2.7, -2) -- ++ (0, 4) node [fill=white, midway, anchor=east] {\rotatebox{90}{Growth direction}};
        \end{tikzpicture}
    }
    \caption[High-resolution microscopy images for the lattice analysis of the merge structures.]{High-resolution microscopy images for lattice analysis. \subref{subfig:merge_region} shows a \acs{df}-\acs{tem_m} image the region immediately after one of the \acs{sio2} spacers between the seed areas (visible in the bottom right of the image). \subref{subfig:merge_qw} shows a high-resolution \acs{bf}-\acs{tem_m} image of the first \acl{qw} (counting from the nucleation areas) in the common part of the template. \subref{subfig:merge_region} was kindly recorded by Michael Stiefel. \subref{subfig:merge_qw} was adapted from the work of Brugnolotto \textit{et al.} \cite{Brugnolotto2023_2} under the terms of the CC BY 4.0 licence \cite{CCBY40}.}
...
Figure~\ref{subfig:merge_region} shows a magnified \acs{bf}-\acs{tem_m} image of the area immediately after the \acs{sio2} spacer between the nucleation regions 2 and 3 as marked in Figure~\ref{subfig:STEM_merge_structure_ov}. These \acs{tem_m} images make structural information readily available highlighting 1D and 2D defects better than their \acs{stem_m}-generated counterparts. This is evident in Figure~\ref{subfig:merge_region} by the presence of a series of lines tracing many of the \hkl{1 1 1} planes in the III-V crystals. These lines are found in correspondence with \acs{rtp}s, which have been a common defect in all samples grown with the \hkl{1 1 1} growth front stabilisation method so far. The absence of other off-plane features in the image suggests the absence of other defects. However, a few defects could still be "hidden" because of the viewing angle.
...
\begin{sidewaysfigure}
    \centering
    \subcaptionbox{
    Precursor sequence for sample 6 \cite{Brugnolotto2023_2}. Each line represents an active flow of the corresponding precursor into the reactor. The colour of the horizontal lines represents the target material. The dashed lines are time-compressed 10 times. The \acl{qw} growth loop was executed six times.
    ...
    % labels and markers for the timescale
     ...
     \subcaptionbox{
        \acs{bf}-\acs{stem_m} image of a nanowire grown with the recipe in \subref{subfig:recipe6} \cite{Brugnolotto2023_2}. Each material layer is labelled, and a guide highlighting the lattice rotation is present. This Figure was adapted from the work of Brugnolotto \textit{et al.} \cite{Brugnolotto2023_2} under the terms of the CC BY 4.0 licence \cite{CCBY40}.
        ...
        \begin{tikzpicture}
            ...
            \draw[white] (-7.2, -1) -- ++ (-87:1);
             (3,-0.5) -- ++ (3:7.5);
            \draw[red, dashed, -stealth] (3,-0.8) -- ++ (3:7.5);
            \draw [red, stealth-] (4, -3) -- ++ (-5, 0) node [midway, anchor=south] {Growth direction};
        \end{tikzpicture}
    }
    \caption[\acs{mocvd} precursor sequence and resulting nanowire for sample 6.]{\subref{subfig:recipe6} \acs{mocvd} precursor sequence and \subref{subfig:s6_wire_OV} resulting nanowire for sample 6 with \acs{eds} linescan path marked by a red dashed arrow.}
...
\begin{figure}
    ...
    \subcaptionbox{
        ...
        \begin{tikzpicture}
            \node[inner sep=0pt] (image) at (0,0) {\includegraphics[width=0.48\textwidth]{4_Properties/Fig/s6_well1.pdf}};
            
            \node [fill=white] at (-3, 3) {\acs{inp}};
            \node [fill=white] at (1.2, 3) {\acs{ingaas}};
            \node [fill=white] at (3, 3) {\acs{inp}};
        \end{tikzpicture}
    }
    \caption[High-resolution \acs{bf}-\acs{stem_m} images of sample 6.]{High-resolution \acs{bf}-\acs{stem_m} images of \subref{subfig:s6_si-IIIV_interface} the nucleation area, showing the \acs{SI} / \acs{ingaas} heterointerface. The red lines highlight the \hkl{1 1 1} atomic planes in \acs{ingaas} and \acs{SI}. As can be seen by comparing the distance between the lines following the atomic planes in \acs{ingaas} and in \acs{SI} at the top and at the bottom of the image, there is a difference in alignment resulting in the rotation of the \hkl<1 1 1> axis highlighted in Figure~\ref{subfig:s6_wire_OV}. \subref{subfig:s6_well1} shows the first \acl{qw} in sample 6, various \acs{rtp}s are visible.}
    \label{fig:s6_HR}
\end{figure}
...
\begin{figure}
    ...
    \caption[Compositional data resulting from an \acs{eds} linescan across the last 6 \acl{qw} of sample 6.]{\acs{eds} linescan compositional data (in percentage) for the \subref{subfig:s6_III_30deg_linesc} III group elements and \subref{subfig:s6_V_30deg_linesc} V group elements 
    across the last \num{6} \acl{qw}s of sample 6 (Figure~\ref{subfig:s6_wire_OV}). The origin of the x-axis is situated before the first well and is the closest point to the \acs{SI} seed. Higher position numbers (in nm) represent the scan moving along the \hkl{1 1 1} vector perpendicular to the growth front away from the seed. This Figure was adapted from the work of Brugnolotto \textit{et al.} \cite{Brugnolotto2023_2} under the terms of the CC BY 4.0 licence \cite{CCBY40}.}
...
\begin{figure}
    \centering
    \begin{tikzpicture}
        \node[inner sep=0pt] (image) at (0,0) {\includegraphics[width=\textwidth]{4_Properties/Fig/s6_competitive_OV.pdf}};
        
        \draw[decorate, decoration={brace}] (-3.7, 2.9) -- ++ (4.2, 0) node[midway, anchor=south]{\acs{ingaas} wells};
        \draw[decorate, decoration={brace}] (2.9, 2.9) -- ++ (3, 0) node[midway, anchor=south]{\acs{ingaas} wells};

        \node [white] at (1.5, 0) {\acs{inp}};
        \node [white] at (-4.5, 0) {\acs{inp}};
        \node at (-7.2, 0) {\acs{SI}};
        \node at (-6, 3.2) {Template \acs{sio2}};
        \node at (-0.5, -4) {Buried \acs{sio2}};

        \draw [red, dashed] (4.8, -2.1) rectangle (6.2, -0.7);
        \node (loss) at (5, -3) {\hkl{1 1 1} stabilisation loss};
        \draw [-stealth] (5.5, -2.1) -- (loss);

        \draw [red, stealth-] (2, -3.3) -- ++ (-5, 0) node [midway, anchor=south] {Growth direction};
    \end{tikzpicture}
    \caption[\acs{bf}-\acs{stem_m} image of a nanowire from sample 6 grown in an area of the wafer with many parasitic nucleations.]{\acs{bf}-\acs{stem_m} image of a nanowire from sample 6 grown in an area of the wafer with many parasitic nucleations. The wire grew from the \acs{SI} seed on the left in the direction specified by the red arrow. Compared with the wire shown in Figure~\ref{subfig:s6_wire_OV} all layers are thinner, resulting in a shorter wire. A loss stabilisation of the \hkl{1 1 1} single-facet growth front occurs at the end of the wire, as revealed by the trace left by the \acs{ingaas} wells.}
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \node at (-7.5, 0.5) {\acs{SI}};
        \node [white] at (-4.5, 0.5) {\acs{inp}};

        \draw [stealth-] (-2.3, -0.7) -- ++ (0, -0.5) node [anchor=north] {\acs{inas}};
        \draw [stealth-] (-1.17, -0.7) -- ++ (0, -1) node [anchor=north] {\acs{ingaas}};
        \draw [stealth-] (-0.1, -0.7) -- ++ (0, -0.5) node [anchor=north] {\acs{gaas}};

        \draw [stealth-] (0.95, -0.7) -- ++ (0, -1) node [anchor=north] {\acs{inas}};
        \draw [stealth-] (2.1, -0.7) -- ++ (0, -0.5) node [anchor=north] {\acs{ingaas}};
        \draw [stealth-] (3.17, -0.7) -- ++ (0, -1) node [anchor=north] {\acs{gaas}};

        \draw [stealth-] (4.25, -0.7) -- ++ (0, -0.5) node [anchor=north] {\acs{inas}};
        \draw [stealth-] (5.37, -0.7) -- ++ (0, -1) node [anchor=north] {\acs{ingaas}};
        \draw [stealth-] (6.46, -0.7) -- ++ (0, -0.5) node [anchor=north] {\acs{gaas}};
        
        \draw [red, stealth-] (3, -2.7) -- ++ (-5, 0) node [midway, anchor=south] {Growth direction};
    \end{tikzpicture}
    \caption[\acs{bf}-\acs{stem_m} image of a nanowire from sample 7.]{\acs{bf}-\acs{stem_m} image of a nanowire from sample 7. The \acs{SI} seed is on the left of the image, followed by the \acs{ingaas} nucleation layer and \acs{inp} stabilisation layer. Of the \num{9} \acl{qw}s, the thickest are composed of \acs{inas}, then the \acs{ingaas} \acl{qw}s, and finally the \acs{gaas} \acl{qw}s are the thinnest, assuming the intended material layers have been deposited. This Figure was adapted from the work of Brugnolotto \textit{et al.} \cite{Brugnolotto2023_2} under the terms of the CC BY 4.0 licence \cite{CCBY40}.}
    \label{fig:s7_OV}
\end{figure}
...
\begin{sidewaysfigure}
    ...
    \caption[III and V concentration profiles across the 7th, 8th, and 9th \acl{qw} of the nanowire cross section cut from sample 7.]{\subref{subfig:s7_III_EDS} III and \subref{subfig:s7_V_EDS} V concentration profiles across the 7th, 8th, and 9th \acl{qw} of the nanowire in Figure~\ref{fig:s7_OV}. This Figure was adapted from the work of Brugnolotto \textit{et al.} \cite{Brugnolotto2023_2} under the terms of the CC BY 4.0 licence \cite{CCBY40}.}
    \label{fig:s7_EDS}
\end{sidewaysfigure}
...
Therefore, the formulae \acs{inas}, \ce{In0_.75Ga0_.35As0_.60P0_.40}, and \ce{In0_.55Ga0_.45As0_.5P0_.5} better identify the three arsenide materials. Longer hold steps could solve the alloying issue in the layers of the last two materials \cite{Brugnolotto2023_2}.
...
\begin{figure}
    ...
    \subcaptionbox{
        ...
        \tikzsetnextfilename{s7_well1}
        \begin{tikzpicture}
            \node[inner sep=0pt] (image) at (0,0) {\includegraphics[width=0.30\textwidth]{4_Properties/Fig/s7_well1.pdf}};

            \node [fill=white] at (-1.9, 2) {\acs{inp}};
            \node [fill=white] at (-0.5, 2) {\acs{inas}};
            \node [fill=white] at (1.9, 2) {\acs{inp}};

            \draw [red, stealth-] (2.4, -2.7) -- ++ (-3.5, 0) node [midway, anchor=north] {Growth direction};
        \end{tikzpicture}
    }
    \subcaptionbox{
        ...
        \tikzsetnextfilename{s7_well2}
        \begin{tikzpicture}
            \node[inner sep=0pt] (image) at (0,0) {\includegraphics[width=0.30\textwidth]{4_Properties/Fig/s7_well2.pdf}};

            \node [fill=white] at (-1.9, 2) {\acs{inp}};
            \node [fill=white] at (-0.5, 2) {\ce{InGaAsP}};
            \node [fill=white] at (1.9, 2) {\acs{inp}};
    ...
    \subcaptionbox{
        ...
        \tikzsetnextfilename{s7_well3}
        \begin{tikzpicture}
            \node[inner sep=0pt] (image) at (0,0) {\includegraphics[width=0.30\textwidth]{4_Properties/Fig/s7_well3.pdf}};
...
\begin{figure}
    ...
    \caption[\acs{gpa} analysis from \acs{bf}-\acs{stem_m} images of the first three \acl{qw}s of the nanowire cross-section cut from sample 7.]{\acs{gpa} analysis from \acs{bf}-\acs{stem_m} images of the first three \acl{qw}s of the nanowire pictured in Figure~\ref{fig:s7_OV}. \subref{subfig:s7_well1} shows the first \acs{inas} \acl{qw}, \subref{subfig:s7_well2} shows the first \ce{In0_.75Ga0_.35As0_.60P0_.40} \acl{qw}, and \subref{subfig:s7_well3} shows the first \ce{In0_.55Ga0_.45As0_.50P0_.50} \acl{qw}. The \acs{gpa} calculations were kindly carried out by Marilyne Sousa.}
    \label{fig:s7_GPA}
\end{figure}
...
\begin{figure}
    ...
    \subcaptionbox{
        ...
        \tikzsetnextfilename{s8_III_map}
        \begin{tikzpicture}
            \node[inner sep=0pt] (image) at (0,0) {\includegraphics[width = 0.47\textwidth]{4_Properties/Fig/s8_III_map.pdf}};

            \draw [white, stealth-] (-3, -0.8) -- ++ (0, -0.5) node [white, anchor=north]{\acs{ingaas}};
            \draw [white, stealth-] (0.7, -0.8) -- ++ (0, -0.5) node [white, anchor=north]{\acs{inp} layer end};

            \draw [red, stealth-] (2.4, -2.7) -- ++ (-3.5, 0) node [midway, anchor=north] {Growth direction};
        \end{tikzpicture}
    }
    \subcaptionbox{
        V-element \acs{eds} map. Red \acs{as}, green \acs{sb}, blue \acs{p}/\acs{pt}.
        \label{subfig:s8_V_map}
    }{
        \tikzsetnextfilename{s8_V_map}
        \begin{tikzpicture}
            \node[inner sep=0pt] (image) at (0,0) {\includegraphics[width = 0.47\textwidth]{4_Properties/Fig/s8_V_map.pdf}};
    ...
    \caption[\acs{eds} overview maps of sample 8.]{\acs{eds} overview maps of sample 8. The wires grew from left to right, as highlighted by the red arrow at the bottom of \subref{subfig:s8_III_map} and \subref{subfig:s8_V_map}. \subref{subfig:s8_III_map} provides the most information on the structure of the wire. The \acs{ingaas} seed, revealed on the bottom left by the presence of \acs{ga}, has a low elongated profile, however the next \acs{inp} layer, revealed by the \acs{in} signal, completely fills the template showing indications of \hkl{1 1 1} stabilisation. As \acs{sb} is introduced in the reactor, however, organised growth breaks down, and the wire ends with a random facet configuration. \subref{subfig:s8_V_map} highlights the \acs{ingaas} seed location, however, either due to the remains of an \acs{sb} droplet or closeness in \acs{eds} peak, the phosphide and antimonide layers are not distinguishable.}
...
\begin{sidewaysfigure}
    \centering
    \subcaptionbox{
    Precursor sequence for sample 9. Each line represents an active flow of the corresponding precursor into the reactor. The colour of the horizontal lines represents the target material. The dashed lines are time-compressed 10 times. The \acl{qw} growth loop was executed six times.
    ...
    % lines
            ...
            \draw [cb1_light_blue, ultra thick, dashed] (11.2, 0) -- (12.2, 0); %10
            ...
            \draw [cb1_light_blue, ultra thick, dashed] (11.2, -1.5) -- (12.2, -1.5); %/10
    ...
    % labels and markers for the timescale
        ...
        \end{scope}
        \begin{scope} [shift={(15.4cm, -0.25)}] % +0.9
            \draw [gray, ultra thick] (0, 0.5) -- (0.5, 0.5) node[anchor = west, text=black] {Hold Step};
            \draw [cb1_orange, ultra thick] (0, 0) -- (0.5, 0) node[anchor = west, text=black] {\acs{ingaas}};
            \draw [cb1_dark_blue, ultra thick] (0, -0.5) -- (0.5, -0.5) node[anchor = west, text = black] {\acs{inp} (intrinsic)};
            \draw [cb1_light_blue, ultra thick] (0, -1) -- (0.5, -1) node[anchor = west, text = black] {\acs{inp} (\acs{sn}-doped)};
            \draw [dashed, ultra thick] (0, -1.5) -- (0.5, -1.5) node[anchor = west, text = black] {\num{0.1} time scale};
            \draw (-0.3, 1) -- (3.6, 1) -- (3.6, -2.15) -- node[midway, fill = white] {Legend} (-0.3, -2.15) -- cycle;
        \end{scope}
        \end{tikzpicture}
    }
    \subcaptionbox{
        \acs{bf}-\acs{stem_m} image of a lamella cut from a nanowire grown with recipe 9 with labelled material layers. The wire grew from the \acs{SI} seed in the direction marked by the red arrow.
        \label{subfig:s9_OV}
    }{
        \tikzsetnextfilename{s9_OV}
        \begin{tikzpicture}
            \node[inner sep=0pt] (image) at (0,0) {\includegraphics[width = 0.64\textwidth]{4_Properties/Fig/s9_OV.pdf}};
            
            \draw[decorate, decoration={brace, mirror}] (-2.05, -0.6) -- ++ (9, 0) node[midway, anchor=north]{\acs{ingaas} wells \& \acs{sn}-doped \acs{inp} barriers};
            \node at (-7.2, 0.5) {\acs{SI}};
            \node [white] at (-4, 0.5) {\acs{inp}};

            \draw [red, stealth-] (2.4, -2) -- ++ (-3.5, 0) node [midway, anchor=north] {Growth direction};
        \end{tikzpicture}
    }
    \subcaptionbox{
        ...
        \tikzsetnextfilename{s9_well1}
        \begin{tikzpicture}
            \node[inner sep=0pt] (image) at (0,0) {\includegraphics[width = 0.32\textwidth]{4_Properties/Fig/s9_well1.pdf}};

            \node [fill=white] at (-3.1, 3) {\acs{inp}};
            \node [fill=white] at (-0.5, 3) {\acs{ingaas}};
            \node [fill=white] at (2.5, 3) {(\textit{Sn}) \acs{inp}};
            
            \draw [red, stealth-] (3, -2.7) -- ++ (-4, 0) node [fill=white, midway, anchor=north] {Growth direction};
        \end{tikzpicture}
    }
    \caption[\acs{mocvd} precursor sequence and \acs{stem_m} analysis of sample 9.]{\acs{mocvd} precursor sequence and \acs{stem_m} analysis of sample 9. \subref{subfig:recipe9} shows a schematic of the precursor sequence, with colour-coded recipe segments. \subref{subfig:s9_OV} shows a \acs{bf}-\acs{stem_m} overview image of a nanowire cut from sample 9. \subref{subfig:s9_well1} shows a high resolution \acs{bf}-\acs{stem_m} image of the first quantum well of the lamella in \subref{subfig:s9_OV}}
...
\begin{figure}
    ...
    \caption[Compositional data from \acs{eds} linescans across the first \acl{qw} of the nanowire cut from sample 9.]{\acs{eds} linescan compositional data (in atomic percentage) for the \subref{subfig:s6_III_30deg_linesc} III group elements and \subref{subfig:s6_V_30deg_linesc} V group elements  across the first quantum well of sample 9 (shown in Figure~\ref{subfig:s9_well1}).}
...
The III group element composition graph of Figure~\ref{subfig:s9_III_30deg_linesc} reveals a \ce{In0_.75Ga0_.35As} composition for the \acl{qw}. However, the V group element composition graph in Figure~\ref{subfig:s9_V_30deg_linesc} shows very high alloying for the barrier layers, which have a real composition of \ce{InP0_.6As0_.4}. The file containing the growth recipe was double-checked to ensure that the \acs{tbas} flow was cut off during \acs{inp} deposition, and it was found that the recipe was programmed correctly. This left a mystery as to the origin of the high \acl{as} concentration found in this and other \acs{eds} spectra of this sample. The likely explanation is an equipment malfunction.

However, this finding does show that it is not only a pure phosphide layer that can stabilise a single \hkl{1 1 1} growth front within a \acs{TASE} template, but also phosphide arsenides. This result shows that \acl{sn} has no negative effect on growth with a single-facet growth front, making it a good dopant for device applications.
