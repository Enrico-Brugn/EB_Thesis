...
This finding does, however, show that it is not only a pure phosphide layer that can stabilise a single \hkl{1 1 1} growth front inside a \acs{tase} template, but also phosphide arsenides. Overall, this result shows that \acl{sn} has no negative effect on the growth and the stabilisation of a single facet growth front, which makes it a good dopant for device applications.
